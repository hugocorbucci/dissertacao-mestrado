...
frequência alta e estável de lançamentos. A Figura
\ref{fig:freshmeat} mostra a distribuição de projetos por frequência
de lançamento. É possível ver que a grande maioria dos projetos demora
mais de 6 meses para lançar uma nova versão com apenas 17\% dos
projetos lançando versões pelo menos uma vez por semestre.
...
contexto, sustentável pode precisa de um sentido um pouco diferente do
...
cadência de lançamentos é uma prática que permite esse tipo de solução.

\subsection[Excelência técnica]{Contínua atenção à excelência técnica
...
características fortes das comunidades livres. \Large{Usar os dados
  que o Analizo vai rodar}. \normalsize
...
maioria dos projetos que foram iniciados não tiveram continuidade. Nesse sentido,
existe MUITO trabalho realizado que é pouco aproveitado.

Por outro lado, não existe nenhuma análise projeto a projeto que
avalia se as funcionalidades implementadas são realmente
necessárias. Logo, não é possível determinar se os projetos livres
adotam o princípio da simplicidade.
...
gerar já que toda decisão é discutida por todos os envolvidos.
...
gargalo, também reduz-se o ``fator caminhão'' da equipe. ``Fator
caminhão'' é uma expressão para expressar a quantidade de membros da
equipe que precisam se ausentar do projeto para que ele pare. Quanto
maior esse número, maior o conhecimento difundido pelos membros da
equipe e menor o risco desse projeto parar.
...
Ambas soluções aumentam o conjunto de \emph{commiters}, distribui mais
o conhecimento entre a equipe e reduz a aparente concentração
...
\cite{Brooks1975-mme} é menor o que torna a revisão menos crítica.
...
seguinte forma. Para as informações que podem ser obtidas
...
visto na Figura \ref{fig:timeline-retro}.
...
de aumentar as chances de alguém perceber algum problema, a
